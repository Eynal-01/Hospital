...
namespace Hospital.WebUI.Controllers
{
	public class PostController : Controller
	{
		...
		public async Task<IActionResult> BlogSingleDoctor(int postId)
		{
			var post = await Blog(postId);

			return RedirectToAction("BlogSingle", "Doctor", post);
		}
		...
		public async Task<IActionResult> PopularPosts()
		{
			...
			var popularPosts = await _context.Posts.OrderByDescending(d => d.ViewCount).Take(3).ToListAsync();
		...
		public async Task<List<PostsShowViewModel>> GetAll(string departmentId = "")
		{
			var user = await _userManager.GetUserAsync(HttpContext.User);
			var role = await _userManager.GetRolesAsync(user);
			//dynamic user;
