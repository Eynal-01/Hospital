...
namespace Hospital.WebUI.Controllers
{
	public class ChatController : Controller
	{
		...
		public async Task<IActionResult> GetHospitalInfo()
		{
			...
			return Ok(new { info = info[0], patientsCount = patients.Count(), doctorsCount = doctors.Count(), departmentsCount = departments.Count() });
		}
		...
		public async Task<IActionResult> GetCurrentUserNotification()
		{
			...
			return Ok(new { myNotification = myNotification });
		}
		...
		public async Task<IActionResult> UserMessage(string id)
		{
			...
			for (int i = 0; i < myNotification.Count(); i++)
			{
				...
				for (int k = 0; k < notificationList.Count(); k++)
				{
					if (notificationList[k].SenderId == myNotification[i].SenderId)
					{
						isTrue = true;
					}
				}
		...
		[HttpGet]
		public async Task<IActionResult> DeleteNotification(string senderId)
		{
			var currentUser = await _userManager.GetUserAsync(HttpContext.User);
			//var user = await _context.Admins.FirstOrDefaultAsync(d => d.UserName == currentUser.UserName && d.Email == currentUser.Email);
			//var chat = await _context.Chats.Include(nameof(Chat.ReceiverDoctor)).Include(nameof(Chat.ReceiverAdmin)).FirstOrDefaultAsync(c => c.SenderId == user.Id && c.ReceiverId == doctorId || c.ReceiverId == user.Id && c.SenderId == doctorId);
			...
			if (userAdmin != null)
			{
				myNotification = await _context.Notifications.Where(d => d.ReceiverId == userAdmin.Id && d.SenderId == senderId).ToListAsync();
			}
			else
			{
				myNotification = await _context.Notifications.Where(d => d.ReceiverId == userDoctor.Id && d.SenderId == senderId).ToListAsync();
			}
